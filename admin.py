...
# --- MAIN DASHBOARD ---
st.title("í ½íµ¹ï¸ Quiz Master Control")
state = get_state()
phase = state['phase']
...
if phase == "LOBBY":
    ...
    with c1:
        ...
        if st.button("í ½í³¥ Import Questions"):
    ...
    if st.button("âš ï¸ Reset Everything"):
        nuke_data()
        st.rerun()

# --- PHASE: INPUT ---
elif phase == "INPUT":
    st.subheader("í ½í´´ Phase: Collecting Answers")
...
# --- PHASE: RESULTS ---
    ...
    st.subheader("í ½í¿¢ Phase: Results")
